// Importing necessary dependencies
import { createSlice } from "@reduxjs/toolkit";

// Initializing initial state for the deleteProduct slice
const initialState = {};

// Creating a slice of the Redux store for managing product deletion related actions and state
export const deleteProductSlice = createSlice({
  name: "delete", // Name of the slice
  initialState, // Initial state
  reducers: {
    // Reducer function for handling product deletion request
    deleteProductRequest: (state) => {
      state.isLoading = true; // Setting isLoading flag to true to indicate request is in progress
    },
    // Reducer function for handling successful product deletion
    deleteProductSuccess: (state, action) => {
      state.isLoading = false; // Setting isLoading flag to false as request is completed
      state.message = action.payload; // Setting success message received from the action payload
    },
    // Reducer function for handling failed product deletion
    deleteProductFailure: (state, action) => {
      state.isLoading = false; // Setting isLoading flag to false as request is completed
      state.error = action.payload; // Setting error message received from the action payload
    },
  },
});

// Exporting action creators for the defined reducers
export const {
  deleteProductRequest,
  deleteProductSuccess,
  deleteProductFailure,
} = deleteProductSlice.actions;

// Exporting the reducer function to be used in the Redux store
export default deleteProductSlice.reducer;
